...
Export <- FlowData %>%
  ...
  mutate(FlowData, timestep = c(0, diff(as.numeric(FlowData$DateTime_UTC))),
         baseflowexport = bt * timestep,
         quickflowexport = qft * timestep) #%>%
         print(FlowData$quickflowexport)
#  summarize(BaseflowExport_cf = sum(baseflowexport),
...
> A much greater percentage of total water exported left as baseflow than as quickflow. 
...
```{r}
library(lubridate)
DataWithMonth <- FlowData %>%
  mutate(FlowData, Month = month(FlowData$DateTime_UTC))

StormData <- FlowData %>%
  filter(DateTime_UTC >= )
NitrateFlowPlot <- ggplot(FlowData, aes(x = Nitrate_mgL, y = Discharge_m3s)) +
  geom_line()
