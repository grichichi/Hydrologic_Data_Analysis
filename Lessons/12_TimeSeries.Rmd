...
> aggregation: combining multiple sets of data 

> subsampling: if you have a dataset composed of different sampling frequencies, it would not be a good idea to aggregate data -- match the more frequent data to the less frequent data

> interpolation: filling in gaps in dataset 
...
#identical distribution assumes a constant sampling frequency for data
...
Mendotadata <- Mendotadata %>%
  ...
  arrange(sampledate) #sorts by date
...
* **Piecewise constant**: AKA "nearest neighbor" interpolation
* **Linear**: drawing direct lines between two points and placing point exactly in the middle of the line
* **Spline**: looks more curvy; typically doesn't outperform linear, so we'll usually go with that
loess smoothing draws line along data in mathematical fashion
...
> There is no significant trend in the data over time. However, there is a significant decrease in total phosphorus for the months of October and November across time.
...
ClearCreekDischarge.Monthly2 <- ClearCreekDischarge %>%
