...
> Variable 3: secchi depth

> Variable 4: chlorophyll levels

> Variable 5: dissolved organic carbon?
...
NewData <- LAGOSnutrient %>%
  select(no2no3, nh4, doc, chla, tp, tn)
...
Correlation <- cor(NewDataDropNA)
CorrelationPlot <- corrplot(Correlation, type = "upper", method = "ellipse")
print(CorrelationPlot)
```

What patterns do you see?

> There seems to be a high and positive correlation between total nitrogen and total phosphorus and dissolved organic carbon. 
There is a moderate and positive correlation between total nitrogen and NO2NO3 and chlorophyll levels. 
There is a high and positive correlation between total phosphorus and chlorophyll levels. 
There is a moderate and positive relationship between total phosphorus and NO2NO3.

2. Create two linear regression models (hint: function `lm`) to predict TN and TP using your five predictor variables. 

```{r}
no2no3_tnTest <- lm(tn ~ no2no3, NewDataDropNA)
summary(no2no3_tnTest)

no2no3_tpTest <- lm(tp ~ no2no3, NewDataDropNA)
summary(no2no3_tpTest)

nh4_tnTest <- lm(tn ~ nh4, NewDataDropNA)
summary(nh4_tnTest)

nh4_tpTest <- lm(tp ~ nh4, NewDataDropNA)
summary(nh4_tpTest)

doc_tnTest <- lm(doc ~ tn, NewDataDropNA)
summary(doc_tnTest)

doc_tpTest <- lm(doc ~ tp, NewDataDropNA)
summary(doc_tpTest)

chla_tnTest <- lm(tn ~ chla, NewDataDropNA)
summary(chla_tnTest)

chla_tpTest <- lm(tp ~ chla, NewDataDropNA)
summary(chla_tpTest)

tp_tnTest <- lm(tp ~ tn, NewDataDropNA)
summary(tp_tnTest)
