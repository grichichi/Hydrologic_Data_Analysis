...
- catchment characteristics & flow patterns
- features within a river -- depth, flow -- & impact on species 
- gis
...
* **Point** <a specific location w/ x,y (lat,lon) coordinates (maybe z & m as well)>
* **Line** <two points connected; simple features objects can hold separate lat,lon measurements in same column -- more of a list object that can hold multiple values>
* **Polygon** <a closed shape; lines connected back to original point>\
* a centroid is a central point of a polygon that is used to tag a location 
...
secchi.summary <- LAGOScombined %>%
  ...
  summarise(secchi.mean = mean(secchi),
            ...
            long = mean(nhd_long)) %>%
  drop_na()
#if want to have area, depth, lat, and lon, need to tell R Studio that... by taking the means (the values are all the same throughout data set, so could use the min or max too)
```
...
```{r}
secchi.spatial <- st_as_sf(secchi.summary, coords = c("long", "lat"), crs = 4326) #crs is the projection selected
#might have to flip around order of "lat" and "long"

Secchiplot <- ggplot() +
  geom_sf(data = states.subset, fill = "white") + #important to plot states first, because white fill would cover the secchi depth data if states plotted second
...
print(Secchiplot)
#a few outliers of deeper secchi depths that are throwing off the scale 
#samples are clustered in places with a lot of lakes 
...
states.subset.maine <- filter(states, ID %in%
                                c("maine"))
states.spatial.maine <- st_as_sf(map(states.subset.maine, coords = c("long", "lat"), crs = 4326))
