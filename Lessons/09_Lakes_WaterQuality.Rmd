NOTE:
to pull a specific file, type: "Git chekcout [commit number]"
Commit number is under repository and commits list
...
- invasive species
- dissolved O2
- nutrients - Nitrogen, Phosphorus, trace nutrients
- microbes
- odor / color
- contaminants (e.g. heavy metals, oragnic contaminants)
...
library(tidyverse)
library(lubridate)
#install.packages("LAGOSNE") <- only have to run this once 
library(LAGOSNE)
library(dplyr)
...
lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE) #<- only have to run this once
...
#NOTE: why are they calling it epilimnion nutrients? 
#Because the epilimnion, the surface layer of the lake, is only layer for which they have info
...
Trophic state == the amount of biomass that a system can support 
concerns: oxygen, public health / safety <-- trophic state (and changes in trophic state over time) can be a proxy for these water quality concerns
secchi disk -- easy and cheap way to measure transparency, which correlates well with some of the other variables 
...
LAGOStrophic <- 
  mutate(LAGOStrophic, 
         TSI.chl = round(10*(6 - (2.04 - 0.68*log(chla)/log(2)))), #round puts value in whole number / int form
...
> A 1:1 relationship would mean that at a given point, both chlorophyll levels and secchi disk depth output the same trophic state index value.
...
> Violin: can reveal more information than a box plot; works best in situation where you have shorter range and higher density; good information about range, median, distribution 

> Bar: conducive to comparisons of different conditions; good for seeing number of observations and for splitting up based on trophic classes or other categories 
...
- different lakes from different areas have different baselines
- if there is deviation from the norm over time (e.g. a lot of eutrophication) this might indicate an issue

- csv file is non-proprietary, so it is a good file type to save 
```{r}
#option, command, i
